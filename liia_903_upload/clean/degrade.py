...
@streamfilter(check=type_check(events.StartRow), fail_function=pass_event, error_function=pass_event)
def degrade_postcodes(event):
    """
    Convert all values that should be postcodes to shorter postcodes
    """
    if event.header == "HOME_POST":
        text = to_short_postcode(event.text)
    elif event.header == "'PL_POST'":
        text = to_short_postcode(event.text)
    else:
        pass
